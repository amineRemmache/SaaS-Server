...
k3s_version="${1:-v1.26.2+k3s1}"; shift
k3s_token="$1"; shift
...
$SHELL -c 'node_name=$(hostname); echo "waiting for node $node_name to be ready..."; while [ -z "$(sudo kubectl get nodes $node_name | grep -E "$node_name\s+Ready\s+")" ]; do sleep 3; done; echo "node ready!"'
...
# NB the default users are generated at https://github.com/k3s-io/k3s/blob/v1.26.2+k3s1/pkg/daemons/control/deps/deps.go#L231-L258
